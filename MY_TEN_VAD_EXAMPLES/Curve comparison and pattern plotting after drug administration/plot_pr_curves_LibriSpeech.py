...
if __name__ == "__main__":
    ...
    # TEN-VAD-TestSet 路径
    # 注意：Windows 路径不要写成普通字符串 "D:\...\testset"，其中 "\t" 会被当作制表符。
    test_dir = Path(r"D:\VAD\1.vad_learn\data_processing\testset")
    ...
    # 获取测试集 wav
    wav_list = sorted([str(p) for p in test_dir.glob("*.wav")])
    ...
    for wav_idx, wav_path in enumerate(wav_list):
        ...
        label_file = wav_path.replace(".wav", ".txt")
    ...
    save_path = f"{script_dir}/PR_Curves_TEN_Silero_FSMN1.png"
